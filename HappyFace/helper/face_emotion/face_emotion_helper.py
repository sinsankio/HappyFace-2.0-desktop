...
class FaceEmotionHelper:
    ...
    def preprocess_face(image: np.ndarray, face_bound_rect: dict, res: tuple[int] = (48, 48)) -> np.ndarray:
        if not FaceEmotionHelper.lm_predictor:
            FaceEmotionHelper.load_lm_predictor()

        face_bound = dlib.rectangle(
            left=face_bound_rect['x'],
            top=face_bound_rect['y'],
            right=face_bound_rect['x'] + face_bound_rect['w'],
            bottom=face_bound_rect['y'] + face_bound_rect['h']
        )
    ...
    @staticmethod
    def get_emotions(image: np.ndarray) -> dict:
        if not (FaceEmotionHelper.fer_model and FaceEmotionHelper.aro_val_model):
